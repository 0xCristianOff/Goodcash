...
- [Development guidelines](#development-guidelines)
    - [General Goodcash Core](#general-goodcash-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Run with the `-testnet` option to run with "play goodcashs" on the test network, if you
...
Goodcash Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/goodcash-project/goodcash/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_goodcash
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_goodcash --log_level=test_suite
$ valgrind -v --leak-check=full src/goodcashd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_goodcash.coverage/index.html`.
```

**Sanitizers**

Goodcash Core can be compiled with various "sanitizers" enabled, which add
...
- ThreadRPCServer : Remote procedure call handler, listens on port 30332 for connections and services them.
...
However, in open source software such as Goodcash Core, where everyone uses
...
pay attention to for reviewers of Goodcash Core code.

General Goodcash Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Goodcash Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of Goodcash Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against Goodcash Core so that they can be integrated
...
file descriptors it will cause problems with Goodcash's `select()` loop, because
...
would be to revert the upstream fix before applying the updates to Goodcash's
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:goodcash-project/goodcash.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `goodcash-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `goodcash-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Goodcash Core, the wallet was always
