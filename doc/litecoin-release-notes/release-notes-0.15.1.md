Goodcash Core version *0.15.1* is now available from:

  <https://download.goodcash.org/goodcash-0.15.1/>
...
A number of changes to the way Goodcash Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 goodcents per kB (0.00001 GCASH). This is to prepare for dropping the minimum transaction fee to 0.1 goodcents per kB (0.0001 GCASH) in the next release.
