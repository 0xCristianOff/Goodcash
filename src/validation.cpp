...
#if defined(NDEBUG)
# error "Goodcash cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Goodcash Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
...

    {
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s GCASH additional fees, %d delta bytes\n",
...
void ThreadScriptCheck() {
    RenameThread("goodcash-scriptch");
